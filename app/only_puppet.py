...
from tkinter import Tk, Frame, LEFT, Label, BOTH, GROOVE, Button, filedialog, PhotoImage, messagebox, ttk
...
class PuppeteerApp:
    ...
    def load_image_from_file(self, file_name):
        ...
        self.source_image_label.configure(image=image, text="", background=self.BGcolor)
    ...
    def update_image(self):
        ...
        if euler_angles is not None and self.source_image is not None:
            ...
            photo_image = PIL.ImageTk.PhotoImage(image=pil_image)
            self.posed_image_label.configure(image=photo_image, text="", background=self.BGcolor)
...
if __name__ == "__main__":
    ...
    poser = MorphRotateCombinePoser256Param6(
        ...
        device=cuda)
    while True:
        RGB = input("select [RGB] for chromekey: \n")
        print("You selected", RGB)
        if RGB != 'R' and RGB != 'G' and RGB != 'B':
            print("Please select from [R, G, B]")
            continue
        break
    ...
    master = Tk()
    PuppeteerApp(master, poser, face_detector, landmark_locator, video_capture, cuda, RGB)
    master.mainloop()
